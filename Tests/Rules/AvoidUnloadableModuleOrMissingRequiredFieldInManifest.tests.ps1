...
BeforeAll {
    ...
    $violationFilepath = Join-Path $assetRoot "TestBadModule\TestBadModule.psd1"
    $violations = Invoke-ScriptAnalyzer $violationFilepath | Where-Object { $_.RuleName -eq $missingMemberRuleName }
    $noViolations = Invoke-ScriptAnalyzer $assetRoot\TestGoodModule\TestGoodModule.psd1 | Where-Object { $_.RuleName -eq $missingMemberRuleName }
    $noHashtableFilepath = Join-Path $assetRoot "TestBadModule\NoHashtable.psd1"
}
...
Describe "MissingRequiredFieldModuleManifest" {
    ...
    Context "Validate the contents of a .psd1 file" {
        It "detects a valid module manifest file" {
            $filepath = Join-Path $assetRoot "TestManifest/ManifestGood.psd1"
        ...
        It "detects a .psd1 file which is not module manifest" {
            $filepath = Join-Path $assetRoot "TestManifest/PowerShellDataFile.psd1"
        ...
        It "detects valid module manifest file for PSv5" {
            $filepath = Join-Path $assetRoot "TestManifest/ManifestGoodPsv5.psd1"
            [Microsoft.Windows.PowerShell.ScriptAnalyzer.Helper]::IsModuleManifest($filepath, [version]"5.0.0") | Should -BeTrue
        }
        ...
        It "detects valid module manifest file for PSv4" {
            $filepath = Join-Path $assetRoot "TestManifest/ManifestGoodPsv4.psd1"
        ...
        It "detects valid module manifest file for PSv3" {
            $filepath = Join-Path $assetRoot "TestManifest/ManifestGoodPsv3.psd1"
    ...
    Context "When given a non module manifest file" {
        It "does not flag a PowerShell data file" {
            Invoke-ScriptAnalyzer `
                -Path "$assetRoot/TestManifest/PowerShellDataFile.psd1" `
